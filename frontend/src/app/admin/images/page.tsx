...
export default function AdminImagesPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Bu görseli silmek istediğinize emin misiniz? (Fiziksel dosya da silinecektir)')) return;
    try {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-gray-800">
        <div>
          <h1 className="text-2xl font-bold font-heading tracking-wider">MEDYA VE GÖRSEL GALERİSİ</h1>
          <p className="text-xs text-gray-400 mt-1">Görselleri doğrudan yükleyin, otomatik WebP dönüşümü ve disk yönetimi</p>
        </div>
        <button
          onClick={fetchImages}
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-lg">
        <h3 className="font-heading font-bold text-base mb-4 flex items-center gap-2">
          <Upload size={18} className="text-emerald-400" /> Yeni Görsel Yükle (Otomatik WebP)
        </h3>
        ...
        <form onSubmit={handleUpload} className="flex flex-col md:flex-row items-stretch md:items-center gap-4 text-xs">
          ...
          >
            {uploading ? 'Dönüştürülüyor...' : 'Yükle & WebP Yap'}
          </button>
        </form>
      </div>
