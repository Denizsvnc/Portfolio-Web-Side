...
export default function AdminAboutPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Bu hakkında bölümünü silmek istediğinize emin misiniz?')) return;
    try {
              ...
              <div className="flex-1 space-y-3">
                ...
                {item.cv_url && (
                  <div className="text-xs text-blue-400 font-bold mt-2">
                    <a href={item.cv_url} target="_blank" rel="noopener noreferrer">CV Dosyası Yüklü</a>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl p-6 shadow-2xl space-y-6 my-8">
            <div className="flex items-center justify-between pb-4 border-b border-gray-800">
              <h2 className="font-heading font-bold text-lg">
                {editingId ? 'Hakkımda Bölümünü Düzenle' : 'Yeni Hakkımda Bölümü Ekle'}
              </h2>
